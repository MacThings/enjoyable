...
- (void)cleanUpInterface {
    ...
    if (row != 1) {
        ...
        if ([self.keyInput.window firstResponder] == self.keyInput) {
            [self.keyInput.window makeFirstResponder:nil];
        }
    }
    
    if (row != 2) {
        [self.mappingPopup selectItemAtIndex:-1];
        if ([self.mappingPopup.window firstResponder] == self.mappingPopup) {
            [self.mappingPopup.window makeFirstResponder:nil];
        }
    ...
    if (row != 3) {
        ...
        if ([self.mouseDirSelect.window firstResponder] == self.mouseDirSelect) {
            [self.mouseDirSelect.window makeFirstResponder:nil];
        }
    } else {
    ...
    if (row != 4) {
        ...
        if ([self.mouseBtnSelect.window firstResponder] == self.mouseBtnSelect) {
            [self.mouseBtnSelect.window makeFirstResponder:nil];
        }
    ...
    if (row != 5) {
        ...
        if ([self.scrollDirSelect.window firstResponder] == self.scrollDirSelect) {
            [self.scrollDirSelect.window makeFirstResponder:nil];
        }
        if ([self.scrollSpeedSlider.window firstResponder] == self.scrollSpeedSlider) {
            [self.scrollSpeedSlider.window makeFirstResponder:nil];
        }
        if ([self.smoothCheck.window firstResponder] == self.smoothCheck) {
            [self.smoothCheck.window makeFirstResponder:nil];
        }
    } else {
...
- (void)loadOutput:(NJOutput *)output forInput:(NJInput *)input {
    ...
    if ([output isKindOfClass:NJOutputKeyPress.class]) {
        ...
        [self.radioButtons selectCellAtRow:2 column:0];
        id mapping = [(NJOutputMapping *)output mapping];
        NSMenuItem *foundItem = nil;
        for (NSMenuItem *item in self.mappingPopup.itemArray) {
            if (item.representedObject == mapping) {
                foundItem = item;
                break;
            }
        }
        [self.mappingPopup selectItem:foundItem];
        self.unknownMapping.hidden = !!foundItem;
...
- (void)focusKey {
    if (self.radioButtons.selectedRow <= 1) {
        [self.keyInput.window makeFirstResponder:self.keyInput];
    } else if ([self.keyInput.window firstResponder] == self.keyInput) {
        [self.keyInput.window makeFirstResponder:nil];
    }
}
...
- (void)mappingListDidChange:(NSNotification *)note {
    ...
    // Passendes Item suchen und auswählen
    NSMenuItem *foundItem = nil;
    for (NSMenuItem *item in self.mappingPopup.itemArray) {
        if (item.representedObject == current) {
            foundItem = item;
            break;
        }
    }
