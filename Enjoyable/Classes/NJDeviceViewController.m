...
- (void)reexpandAll {
    ...
    if (self.inputsTree.selectedRow == -1) {
        ...
        if (row >= 0) {
            [self.inputsTree selectRowIndexes:[NSIndexSet indexSetWithIndex:row] byExtendingSelection:NO];
...
- (void)outlineViewSelectionDidChange:(NSNotification *)notification {
    ...
    NJInputPathElement *item = (row >= 0) ? [outlineView itemAtRow:row] : nil;
    if (item) {
...
- (NJInput *)selectedHandler {
    NSInteger row = self.inputsTree.selectedRow;
    if (row < 0) return nil;
    NJInputPathElement *element = [self.inputsTree itemAtRow:row];
